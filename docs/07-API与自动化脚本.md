...
- 分页、筛选、排序规则。
- 是否幂等。
- 对应 route、service、validator、权限函数和测试请求。
...
更完整的接口设计使用 [API 规格模板](../templates/API规格模板.md)。AI 应先生成 API 规格，再写 route 代码。

## 表格字段自动分析

如果有 Excel / CSV 样例，AI 应先做字段画像，不要直接写导入代码。
...
使用 [表格字段分析报告](../templates/表格字段分析报告.md)。
...
使用 [自动化脚本运行手册](../templates/自动化脚本运行手册.md)。
